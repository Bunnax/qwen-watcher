...
function initPerf() {
  if (!PERF || typeof PERF.init !== 'function') return;
  PERF.init({
    log: function (...args) {
  ...
  if (pair) {
    try {
      ...
      if (Array.isArray(ohlc) && ohlc.length) {
        return { candles: ohlc.map(function (c) { return { start: +(c.timestamp || c.time), open: +c.open, high: +c.high, low: +c.low, close: +c.close, volume: +c.volume }; }), source: 'bitstamp' };
      }
...
function connectUpstream() {
  ...
  upstream.onmessage = function (ev) {
    ...
    // Feed real Coinbase market trades into the canonical PERF engine.
    try {
      const msg = JSON.parse(data);

      if (msg && msg.channel === 'market_trades' && Array.isArray(msg.events)) {
        for (const event of msg.events) {
          if (!event || !Array.isArray(event.trades)) continue;

          for (const trade of event.trades) {
            const product = trade.product_id || '';
            const price = parseFloat(trade.price);

            if (!product || !Number.isFinite(price) || price <= 0) continue;

            const asset = product.endsWith('-USD')
              ? product.slice(0, -4)
              : product;

            const tradeTs = trade.time
              ? Date.parse(trade.time)
              : Date.now();

            if (
              PERF &&
              typeof PERF.onTrade === 'function'
            ) {
              PERF.onTrade(
                asset,
                price,
      ...
      log('upstream trade parse warning: ' + e.message);
    }

    // Preserve the original Coinbase frame for frontend consumers.
...
wss.on('connection', function (ws) {
  ...
  ws.on('message', function (raw) {
    ...
    try {
      ...
      log('frontend frame parse warning: ' + e.message);
      return;
    }
